...
<body>
  ...
  // Check if profile_ids is set in the URL
  if (isset($_GET['profile_ids'])) {
    ...
    // Redirect back if no profile_ids is found in URL
      ...
      <?php if (!empty($profiles)): ?>
        <?php foreach ($profiles as $profile): ?>
          <table class="user-table mt-5">
            ...
            <tr>
              <th>Role:</th>
              <td><?php echo htmlspecialchars($profile['profile_type']); ?></td> <!-- db.value here-->
            </tr>
            <tr>
              <th>Description:</th>
              <td class="tdtxtview">
